...
# ── Live deploy events ───────────────────────────────────────────
# The deploy flow emits a machine readable event per step so the ML service can
# drive the frontend's deployment timeline.
EVENT_PREFIX = "@@GRIFFIN_EVENT "


def _emit_deploy_step(step: str, status: str, url: str = "") -> None:
    """Emit a live deploy_step event the ML service parses and forwards."""
    try:
        payload = {
            "kind": "deploy_step",
            "step": step,
...
def _generate_ci_workflow(tech_stack: dict) -> str:
    """Return a generic, valid GitHub Actions workflow.

    Picks a Node install-and-build job when the tech stack languages include
    JavaScript or TypeScript, otherwise a Python setup-and-test job.
    """
    languages = tech_stack.get("languages", []) if isinstance(tech_stack, dict) else []
    normalized = {str(lang).lower() for lang in languages}
    is_node = bool(normalized & {"javascript", "typescript"})
...
def devops_office(state: OfficeState) -> dict:
    ...
    # ── Step 0: Add a generated CI workflow before writing files ─
    try:
        ci_path = ".github/workflows/main.yml"
        if ci_path not in codebase:
            codebase[ci_path] = _generate_ci_workflow(state.get("tech_stack", {}))
            logger.info(f"Added generated CI workflow: {ci_path}")
            logs.append(f"[DEVOPS] Added {ci_path}")
    except Exception as e:
        logger.warning(f"Could not add CI workflow: {e}")
    ...
    if OFFLINE or not GITHUB_TOKEN or not GITHUB_OWNER:
        ...
        logger.warning(msg)
        logs.append(msg)
        for _step in ("git_init", "commit", "push"):
            _emit_deploy_step(_step, "skipped")
    else:
        git_step = "git_init"
        try:
            ...
            git_step = "git_init"
            _emit_deploy_step("git_init", "start")
            git_repo = GitRepo.init(project_dir)
            _emit_deploy_step("git_init", "ok")
            ...
            # Push
            git_step = "push"
            _emit_deploy_step("push", "start")
            origin.push(refspec="HEAD:refs/heads/main", force=True)
            _emit_deploy_step("push", "ok", github_url)
            ...
            _emit_deploy_step(git_step, "error")
    ...
    if OFFLINE or not VERCEL_TOKEN:
        ...
        logger.warning(msg)
        logs.append(msg)
        _emit_deploy_step("build", "skipped")
        _emit_deploy_step("deploy", "skipped")
    else:
        print(f"\n  {Fore.MAGENTA}▲ Deploying to Vercel...{Style.RESET_ALL}")
        logger.info("Starting Vercel deployment...")

        _emit_deploy_step("build", "start")
    ...
    # Append an ASCII QR code of the live URL (online path only)
    if vercel_url:
        try:
            import io
            import qrcode
            ...
            report_content += (
                "\n## Live URL QR\n\n"
                f"Scan to open {vercel_url}\n\n"
                "```\n" + buf.getvalue() + "```\n"
            )
            logger.info("Appended ASCII QR code to REPORT.md")
        except Exception as e:
